...
class User:
    ...
    def register(self):
        print("Welcome to the SHAO BANK Registration!")
    ...
    def auth(self):
        ...
        if len(dat.fetchall()) > 0:
            ...
            for row in c.fetchall():
                print("Welcome to the SHAO BANK",row[0],row[1],"Your balance is:", row[2],"₾")
                for_auth = True
            if for_auth:
                self.operation_ask()
        else:
    ...
    def operation_ask(self):
        while True:
            ask = input("Which operation do you want to do? (W/D/T/EXIT/LOGUT): ").lower()
            if ask == "w":
                self.withdraw()
            elif ask == "d":
                self.deposit()
